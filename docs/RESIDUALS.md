...
| 5 | **Multi-node bot-fleet throughput is unmeasured.** Reported numbers are single-host; the cross-pod Indexed-Job scale-out is designed but not benchmarked on a cluster. | Throughput / scale | `--pod-index` gives globally-unique IDs and the Indexed Job template exists (`infra/k8s/31-bot-fleet-job.yaml`); the single-host ceiling (~250k orders/s) is measured. ([BENCHMARK_RESULTS.md](BENCHMARK_RESULTS.md#honest-scope)) | Low | No multi-node cluster was provisioned for the hackathon; the scaling model is documented in [BLUEPRINT.md](BLUEPRINT.md), not claimed as measured. |
